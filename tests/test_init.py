...
async def test_devices(
    ...
    for specs in devices.get_devices.return_value:
        ...
        device = device_registry.async_get_device_by_identifier((DOMAIN, device_id), mock_config_entry.entry_id)
...
async def test_removing_stale_devices(
    ...
    assert not device_registry.async_get_device_by_identifier((DOMAIN, "aaa-bbb-ccc"), mock_config_entry.entry_id)
...
async def test_via_device_nested_hierarchy(
    ...
    base_device = DeviceResponse.from_json(
        load_fixture("devices/virtual_valve.json", DOMAIN)
    ).items[0]
    ...
    mock_smartthings.get_device_status.return_value = DeviceStatus.from_json(
        load_fixture("device_status/virtual_valve.json", DOMAIN)
    ).components
